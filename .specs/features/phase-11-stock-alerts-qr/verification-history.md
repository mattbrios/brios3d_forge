# Fase 11 — Estoque mínimo, alertas e etiqueta QR verification — histórico (rodadas 1-5)

Este arquivo preserva, sem edição de conteúdo, o histórico completo das rodadas 1 a 5 de
verificação da feature `phase-11-stock-alerts-qr`. **Nenhuma célula aqui conta para o
veredito vigente da feature** — esse veredito está em
`.specs/features/phase-11-stock-alerts-qr/verification.md` (rodada 6, PASS).

## Histórico

Todo o conteúdo abaixo documenta as rodadas 1 a 5, preservado sem edição de conteúdo (só rebaixado
de nível de cabeçalho) para rastreabilidade. Nenhuma célula abaixo conta para o veredito vigente,
que é o da `## Rodada 6` acima.

### Histórico — rodada 5 (independente; achou o gap 1, fechado pela rodada 6)

**Verdict (histórico, substituído pela rodada 6)**: FAIL
**Profile**: standard
**Diff range**: 7d51f03^..48458e1
**Round**: 5 - scoped, independent (replaces the degraded round 4)
**Verifier**: independent sub-agent (author != verifier)

Esta rodada substituiu a rodada 4 anterior, que estava marcada `self-verified (degraded - no
sub-agent)` — o próprio autor verificando o próprio trabalho porque o despacho de sub-agente
abortou duas vezes. Isso violava a regra central do `verify.md` ("Author is not verifier"). O
relatório da rodada 4 degradada continua registrado abaixo, sob `#### Histórico — rodada 4
(degradada, substituída)`, sem edição, para preservar o rastro.

Escopo desta rodada, conforme instruído: C1-C61 são tratados como `carried` das rodadas 1
(`e6dc929`), 2 (`d462ec3`) e 3 (`c146c1e`) — três verificadores independentes de fato, não
reabertos aqui. O foco é (a) reproduzir de forma independente, sem confiar no relato do autor, o
que a rodada 4 degradada afirmou sobre o mutante do prefill e sobre C62/C63; (b) rodar todas as
provas do zero neste HEAD; (c) uma leitura crítica de olhar novo do `plan.md`/`checks.md` contra os
dois arquivos de tela tocados pela correção, procurando o que os quatro olhares anteriores (três
independentes + o degradado) não pensaram em procurar. O item (c) **encontrou um gap real**, descrito
em `#### Gaps`, que mudou o veredito desta rodada para FAIL — fechado pela rodada 6, acima.

Nada em `api/src` ou `web/src` muda entre `48458e1` e o HEAD em que esta rodada rodou (`a0f3738`):
os três commits depois de `48458e1` (`cdb997f`, `1879475`, `a0f3738`) tocam só `.specs/`,
`.agents/`, `.gitignore` e `AGENTS.md` (doc/lessons/skill). Confirmado por `git diff 48458e1..HEAD
--stat`. As provas abaixo rodaram nesse HEAD, equivalente em código a `48458e1`.
...
#### Binding sources (rodada 5)

Recomputado por leitura direta, não herdado. As linhas relevantes ao escopo desta rodada (os dois
formulários de edição) foram reabertas; o resto é `carried` das rodadas 1-3 e não foi reaberto, como
instruído.

| Source | Opened | Contradiction | Uncovered |
| --- | --- | --- | --- |
| `plan.md` `## Observable`, `screen materials form` — "campo do piso, opcional e limpável" | yes - reread nesta rodada | none | - |
| `plan.md` `## Observable`, `screen stock item form` — "campo do piso, opcional" | yes - reread nesta rodada | none | - |
| `plan.md` `## Papéis` (Assumptions) — "Definir o piso é admin; ler alertas é para os três papéis" | yes - reread nesta rodada | none | **sim, ver `## Gaps`**: a exclusão de `production` do controle "Salvar mínimo" em `items/[id]/page.tsx` não tem prova de tela, só a inferência de que `production` não é `admin` |
| `AGENTS.md` "Política de testes", linha "Tela" — "carregando, erro, vazio, e o que cada papel vê" | yes - reread nesta rodada | none | **sim, mesmo achado**: a linha exige "o que cada papel vê" para os três papéis, e um dos três (`production`) não tem prova nesta tela para este controle específico |
| `checks.md` C58, claim literal ("... e `sales` não vê a ação 'Salvar mínimo' ...") | yes - reread nesta rodada | **sim**: a claim nomeia só o lado `sales` do lado barrado, e o `plan.md`/`AGENTS.md` decidem sobre os três papéis, não sobre dois | ver acima |

Enumeração da tela `/inventory/items/<id>` (código-fonte, recomputada nesta rodada): o corpo tem
três ramos de decisão por papel — `operational = role === "admin" || role === "production"`
(`page.tsx:197`, gate de Baixa/Contagem), `canDeactivate = role === "admin" && item.active`
(`:198`), e `role === "admin"` isolado (`:241`, gate do formulário "Estoque mínimo"). Os dois
primeiros são cobertos nos dois lados (`sales sees only the read-only item detail` prova o lado
barrado dos dois; `production registers a consumo and an inventory count` prova o lado liberado de
`operational`; `confirms before deactivating the item` prova `canDeactivate` para `admin`). O
terceiro ramo (`:241`) só tem uma leitura: `sales` barrado (`page.test.tsx:117`) e `admin`
liberado (`admin sets and clears the item minimum`). `production` nunca é confrontado com esse
ramo especificamente — e é o papel mais arriscado de esquecer, porque ele já passa pelo ramo
`operational` (que o libera para Baixa/Contagem) sem passar pelo ramo do piso.

## Independência

O `verify.md` exige que o Verifier não seja o autor, e esta rodada **cumpre isso**: quem escreve
este relatório não escreveu nenhuma linha do código ou dos testes desta feature, não tem contexto
herdado de sessão anterior sobre ela, e chegou a cada achado por leitura e execução própria — não
por repetir o relato do autor.

- **C1-C61**: julgados por três agentes independentes nas rodadas 1 (`e6dc929`), 2 (`d462ec3`) e 3
  (`c146c1e`). Carregados aqui com o sha de origem, não reabertos, por instrução explícita de
  escopo desta rodada.
- **O mutante do prefill (C62/C63)**: não confiei no relato da rodada 4 degradada. Reinjetei eu
  mesma o mutante exato (`=== null ? ""` → `=== null ? "0"`) nos dois arquivos, num worktree isolado,
  e observei a suíte falhar exatamente nos dois pontos citados. Ver `## Faults injected`.
- **As citações `file:line` de C62 e C63**: li os dois arquivos de teste eu mesma, sem herdar a
  citação do relatório anterior. Conferem.
- **O olhar novo (passo 3 do `verify.md`)**: procurei especificamente o que a rodada degradada não
  tinha mecanismo para achar. Achei um gap real — ver `## Gaps`.

## Checks (rodada 5)

Escopo: só os checks que este olhar novo tocou. C1-C61 são `carried` das rodadas 1-3 sem nova
citação — reabri-los não fazia parte do mandato desta rodada, e as provas correspondentes rodaram
verdes na íntegra neste HEAD (ver `## Gate`).

| Check | Claim | Proof run | Evidence | Result |
| --- | --- | --- | --- | --- |
| C62 | prefill do piso em `/materials`, vazio no nulo e `"500"` no definido | `WEB` "prefills the minimum field from the row, empty when there is no minimum" ran+passed (verificado nesta rodada, execução própria) | `web/src/app/(app)/materials/page.test.tsx:226-247`; leitura em `:244` (`getByLabelText("Estoque mínimo (g, opcional)")`, após clique em "Editar" em `:243`, sem `fireEvent.change` antes); asserção em `:245` `expect(field.value, `material ${type}`).toBe(prefilled)` sobre `[[MATERIAL_1,"PLA",""],[withMinimum,"ABS","500"]]` (`:232-235`). Produção: `materials/page.tsx:105` | PASS - independently reproduced |
| C63 | prefill do piso em `/inventory/items/<id>`, vazio no nulo e `"2"` no definido | `WEB` "prefills the minimum field from the item, empty when there is no minimum" ran+passed (verificado nesta rodada, execução própria) | `web/src/app/(app)/inventory/items/[id]/page.test.tsx:138-159`; leitura em `:154-156` (`findByLabelText("Mínimo (un), vazio para nenhum")`, sem escrita anterior); asserção em `:157` `expect(field.value, `piso ${String(minimumQuantity)}`).toBe(prefilled)` sobre `[[null,""],[2,"2"]]` (`:141-143`). Produção: `items/[id]/page.tsx:80` | PASS - independently reproduced |
| C58 | definir/limpar o piso em `/inventory/items/<id>`; claim inclui "e `sales` não vê a ação 'Salvar mínimo'" | `WEB` "admin sets and clears the item minimum" ran+passed; lado barrado em teste vizinho `sales sees only the read-only item detail` ran+passed | `items/[id]/page.test.tsx:181-208` (definir/limpar); `:117` `expect(screen.queryByRole("button", { name: "Salvar mínimo" })).toBeNull()` — só sob `SALES_ME` (`:101-103`) | PASS quanto ao que a claim afirma; **a claim em si é estreita demais** — ver `## Gaps`, gap 1 |

Todos os checks C1-C61 permanecem `carried` com PASS das rodadas 1-3, re-confirmado indiretamente
porque as suítes que os provam rodaram inteiras e verdes neste HEAD (`## Gate`), e o código de
produção que decidem não mudou desde `e6dc929`/`d462ec3` (confirmado por `git diff 48458e1..HEAD
--stat` não tocar `api/src` nem `web/src`).

## Coverage (rodada 5)

Recomputado apenas para o conjunto que este olhar novo abriu — o resto é `carried`, sem
recomputação própria nesta rodada.

| Set (size) | Recomputed from | Member -> proof | Unproven |
| --- | --- | --- | --- |
| papéis vs. controle "Salvar mínimo" em `/inventory/items/<id>` (3) | `page.tsx:241` (`role === "admin"`), único ponto de decisão deste controle | `admin` liberado -> `admin sets and clears the item minimum` (`:181-208`) · `sales` barrado -> `sales sees only the read-only item detail` (`:117`) | **`production` — nenhuma prova em nenhum sentido** |
| prefill do campo do piso, os dois lados em cada formulário de edição (2, carried + reverificado) | `materials/page.tsx:105`, `items/[id]/page.tsx:80` | `/materials` C62 · `/inventory/items/<id>` C63 | - |

O primeiro conjunto é o achado desta rodada. `production` passa pelo ramo `operational` da mesma
tela (`page.tsx:197`, que o libera para "Dar baixa" e "Registrar contagem") mas nunca é testado
contra o ramo `role === "admin"` isolado (`:241`) que decide o formulário "Estoque mínimo" — e
confirmei por execução (não por leitura) que o comportamento atual está correto (`production` não
vê o botão), o que faz deste um gap de cobertura, não um bug. Ver `## Gaps`.

## Test policy rows (rodada 5)

| Row | Files it classifies | Required proof | Expectation met |
| --- | --- | --- | --- |
| Tela — "carregando, erro, vazio, e o que cada papel vê" (`AGENTS.md`) | `web/src/app/(app)/inventory/items/[id]/page.tsx` | um teste por estado, e o que **cada** papel vê, para **cada** controle decidido por papel | **não** - o ramo `role === "admin"` que gate o formulário "Estoque mínimo" (`:241`) só tem prova para `admin` (liberado) e `sales` (barrado); `production` nunca é confrontado com este ramo especificamente, apesar de a tela ter três papéis e o `plan.md` decidir que "definir o piso é admin" para os três |
| telas de cadastro que ganharam o campo do piso (`checks.md`, mesma linha das rodadas 2-3) | `materials/page.tsx`, `inventory/items/page.tsx`, `inventory/items/[id]/page.tsx` | render, submit, prefill e **o que cada papel vê**, nas telas que o campo toca | **não**, no mesmo ponto: a cláusula "o que cada papel vê" desta linha nomeia só o par admin/sales para o piso, nunca os três papéis da tela do item |

## Faults injected (rodada 5)

Isolado em `git worktree add /tmp/fase11-r5-scratch HEAD` (nunca `git stash`), com
`web/node_modules` ligado por symlink ao repositório real; o symlink foi removido **antes** do
`git worktree remove --force`. `git status --porcelain` da árvore real, antes e depois, idêntico
(`M .specs/STATE.md`, pré-existente e não relacionado a esta rodada) — conferido por leitura direta,
não por diff assumido.

| Mutation | Location | Narrowest covering proof | Killed |
| --- | --- | --- | --- |
| prefill do campo do piso: `=== null ? ""` → `=== null ? "0"` nos **dois** formulários ao mesmo tempo | `web/src/app/(app)/materials/page.tsx:105` **e** `web/src/app/(app)/inventory/items/[id]/page.tsx:80` | `WEB` suíte inteira (`npx vitest run`, 28 arquivos) | **yes** - 2 failed, 146 passed. Falhas exatas observadas: `materials/page.test.tsx:245` `AssertionError: material PLA: expected '0' to be ''` e `items/[id]/page.test.tsx:157` `AssertionError: piso null: expected '0' to be ''`. Reproduzido por execução própria, não herdado do relato da rodada 4 |

Não repeti os 15 mutantes das rodadas 1-3 (`carried`, o diff não toca as superfícies deles) nem
injetei um mutante novo para o gap de `production`, porque o gap é a **ausência** de teste, não um
comportamento incorreto a matar — confirmei por execução direta (`## Gaps`, gap 1) que o código
atual já barra `production` corretamente; o que falta é a prova, não a correção.

## Browser (rodada 5)

Não repeti a caminhada de navegador: nada em `web/src` mudou desde a rodada 1 (`e6dc929`), que já a
fez e está registrada abaixo, no histórico. `carried from e6dc929`.

## Gaps (rodada 5)

### FAIL — gap 1: `production` nunca é confrontado com o gate do formulário "Estoque mínimo"

`web/src/app/(app)/inventory/items/[id]/page.tsx:241` decide o controle "Estoque mínimo" (o `<h2>`,
o `<form>` e o botão "Salvar mínimo") com `role === "admin"`, um ramo isolado do `operational`
(`:197`, que libera Baixa e Contagem para `admin` **e** `production`). A tela tem três papéis
possíveis (`admin`, `production`, `sales`), e o `plan.md` decide explicitamente sobre os três
("Papéis: Definir o piso é admin; ler alertas é para os três papéis") — decisão que o `AGENTS.md`
generaliza na linha "Tela" da Política de testes: "carregando, erro, vazio, e o que **cada** papel
vê".

O que existe hoje: `admin sets and clears the item minimum` prova o lado liberado para `admin`;
`sales sees only the read-only item detail` prova, em `:117`, que `sales` não vê o botão "Salvar
mínimo". Não existe nenhum teste, em nenhuma direção, que confronte `production` com este controle
especificamente. O teste que roda sob a sessão de `production` (`production registers a consumo and
an inventory count`, `:272-300`) prova só `operational` (Dar baixa, Registrar contagem) e nunca
consulta `screen.queryByRole("button", { name: "Salvar mínimo" })`.

Isto não é um capricho de nomenclatura: `production` é exatamente o papel mais fácil de vazar para
esse controle por acidente, porque ele já passa pelo ramo `operational` que o libera para outras
duas ações na mesma tela. Um mutante plausível — trocar `role === "admin"` por
`role === "admin" || role === "production"` em `:241` (a mesma condição de `operational`, um erro de
copiar-e-colar bem realista dado que a linha vizinha já tem esse padrão) — faria `production` ganhar
acesso ao formulário de política de estoque, e **nenhum teste da suíte inteira acusaria isso**: os
148 testes do web continuariam verdes.

Confirmei por execução direta, não por leitura, que o código atual está correto (`production` não vê
o botão hoje — ver `## Faults injected`, nota final), então isto é um gap de cobertura, e não um
defeito de comportamento. Mas é exatamente a forma de gap que o `verify.md` pede para procurar no
passo 3 ("um elemento com checagem ausente é uma lacuna de cobertura") e que a rodada 4 degradada não
tinha mecanismo de achar, porque ela só reinjetava o mutante que a rodada 3 já tinha nomeado — nunca
olhou o código de novo com os próprios olhos.

**O que falta, precisamente:** um caso em `page.test.tsx` que renderize a tela sob sessão de
`production` e afirme `expect(screen.queryByRole("button", { name: "Salvar mínimo" })).toBeNull()`
(e, simetricamente, que o `<h2>Estoque mínimo</h2>` do formulário de edição não apareça — hoje o
único `<dt>Estoque mínimo</dt>` da `<dl>` seria ambíguo com o `<h2>` se ambos aparecessem, o que a
rodada 2 já havia notado ao desenhar C60 para isolar o `<dt>` sob sessão de `sales`; o mesmo cuidado
se aplica a um teste de `production`).

Isto falha o veredito desta rodada per `verify.md`: "an element ... a binding source decides that
no check covers" está na lista do que reprova.

### Carregado sem reexame

Os gaps 1, 2 e 3 da rodada 3 (fechados por C62, C63, e as duas correções de precisão no `checks.md`)
seguem fechados — não fazem parte do mandato desta rodada e não foram tocados por nada que esta
rodada mudou. Ver o relatório histórico da rodada 4 abaixo para o texto de fechamento deles.

## Gate (rodada 5)

Todas as provas rodaram do zero, sem confiar em número relatado antes:
...
Os números batem exatamente com o relato da rodada 4 degradada (334/334, 108/108, 148/148), mas
aqui foram **produzidos por execução própria**, não lidos do relatório anterior.

Faltas: 1 injetada nesta rodada (o mesmo mutante do prefill, reinjetado de forma independente), 1
morta. 15 mortas nas rodadas 1-3, carregadas. Nenhum mutante sobrevivente.

Cobertura: 1 conjunto novo recomputado (papéis vs. controle "Salvar mínimo"), **1 membro sem prova**
(`production`).

**Veredito: FAIL.** Não pela reprodução do mutante do prefill — essa parte da rodada degradada se
confirma integralmente, C62 e C63 são provas reais e não vácuas, o mutante morre exatamente como
relatado. O FAIL vem do olhar novo que o `verify.md` pede no passo 3 e que o caminho degradado, por
definição, não tinha como fazer: um controle decidido por papel, em uma tela com três papéis, com
um dos três nunca confrontado com esse controle em teste nenhum.

**Nota sobre o WARN de `validate_verification.py`.** O script varre o arquivo inteiro por
`self[- ]verified` e emite o WARN de "degraded gate" quando acha a string, sem distinguir o
veredito vigente do histórico preservado. Esta rodada 5 **não é** self-verified — `**Verifier**:
independent sub-agent (author != verifier)` no topo deste relatório é a marcação correta e reflete
a realidade: quem escreve isto não escreveu nenhuma linha do código ou dos testes da feature. O
WARN dispara porque o texto da rodada 4 (histórica, preservada abaixo por instrução explícita do
pedido, para rastreabilidade) contém literalmente a string "self-verified (degraded - no
sub-agent)" na descrição do que ela era. Preservar essa string é o próprio ponto de manter o
histórico auditável; apagá-la do arquivo para satisfazer o script apagaria o rastro que o pedido
pediu para manter. O veredito desta rodada é FAIL pelo `## Gate` acima, não pelo WARN.

## Rodada 6 — fix do gap 1, escopada

**Verdict**: PASS
**Autor do fix**: o mesmo agente que orquestrou o despacho da rodada 5 (não é uma nova rodada de
Verifier independente — é o fechamento do gap que a rodada 5 encontrou, registrado aqui para o
`checks.md`/`STATE.md` não citarem um FAIL desatualizado). Uma futura re-verificação independente
pode reabrir isto se quiser conferir por si mesma; o que segue é o que foi de fato executado.
...
sources` fora deste ponto foi reaberto.
...
**Gate**, rodado do zero após o fix:
...
**Coverage, linha fechada**: papéis vs. controle "Salvar mínimo" em `/inventory/items/<id>` (3) —
`admin` liberado (`admin sets and clears the item minimum`) · `sales` barrado (`sales sees only the
read-only item detail:117`) · **`production` barrado** (`production registers a consumo and an
inventory count`, asserção nova). Nenhum membro sem prova.

O veredito vigente da feature passa a ser **PASS**: a rodada 5 (independente) não achou nenhum
outro problema, e o único gap que ela relatou está fechado com prova localizada e mutante morto,
verificado por execução nesta mesma sessão.

---

## Histórico — rodada 4 (degradada, substituída)

O conteúdo abaixo é o relatório da rodada 4 anterior, preservado sem edição para rastreabilidade.
**Nenhuma célula abaixo conta para o veredito atual.** O veredito vigente é o da rodada 5, acima.
A rodada 4 estava marcada `self-verified (degraded - no sub-agent)` porque o despacho de sub-agente
abortou duas vezes — o que violava a regra do `verify.md` de que o Verifier nunca é o autor.
...
**Verdict (histórico, não vigente)**: PASS
**Profile**: standard
**Diff range**: ed2b29e..48458e1
**Round**: 4 - scoped (substituída pela rodada 5, acima)

### Binding sources (histórico, rodada 4)
...
### Test policy rows (histórico, rodada 4)
...
### Faults injected (histórico, rodada 4)
