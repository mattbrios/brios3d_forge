...
4. `MakerWorldClient` (door 2, door 3, door 4) - `GET https://makerworld.com/api/v1/design-service/design/<designId>` com timeout, limite de tamanho e sem seguir redirecionamento. Responde 404 -> `PrintProfileError` 404; qualquer outra falha -> `PrintProfileError` 502
...
| decision | a decisão de nunca processar G-code/3MF vira o AD-010. O padrão de chamada HTTP de saída vira o AD-009, substituído pelo AD-011 (door 4), e as Fases 14, 28 e 29 vão copiá-lo |
| route | nova rota `POST /print-profiles/import`. Nenhuma rota existente muda |
| app | `AppModule` importa o `PrintProfilesModule`. Nenhuma variável de ambiente nova e nenhuma dependência nova (`node:https`, door 4) |
...
| 4. cliente HTTP de saída: `node:https` no lugar do `fetch` (substitui só o cliente da door 3; timeout, redirect, tamanho e host fixo continuam) - descoberto no build, escolhido pelo usuário, AD-011 | `https.get(new URL("/api/v1/design-service/design/<designId>", baseUrl), { headers: { accept: "application/json", "user-agent": "Brios3DForge/<versão>" }, signal: AbortSignal.timeout(10_000) }, …)`, sem seguir redirecionamento (3xx vira falha `redirect`), corpo contado por `data` e destruído acima de 5 MB | `fetch` nativo (door 3): o Cloudflare do MakerWorld responde `403` com `cf-mitigated: challenge` para o `fetch` do Node 24 com qualquer User-Agent, e `200` para `node:https`, `curl` e `wget` com o mesmo User-Agent honesto (verificado em 2026-09-21). Trocar o User-Agent por um de navegador: seria se passar por outro cliente, o que o plano recusa (Assumptions) |
